			...
			<div class="container">
				<div class="row">
					...
					<div id="credit" class="<?php echo has_nav_menu( 'footer_menu' ) && ! standard_is_offline() ? 'span5' : 'span12'; ?>">
						...
						<?php } else { ?>
							<?php printf( __( '&copy; %1$s %2$s &mdash; %3$s by %4$s', 'standard' ), date( 'Y' ), '<a href="' . site_url() . '">' . get_bloginfo( 'name' ) . '</a>', '', '<a href="http://idlefusion.com" target="_blank">Idle Fusion</a>'); ?>
						<?php } // end if/else ?>
